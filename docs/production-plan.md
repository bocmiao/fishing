...
| 方案 C | 行走地图（地形混合、物件遮挡、网格碰撞、镜头跟随）、三朝向行走动画、键鼠和手柄输入 | 中，M7 |
...
| 4 | 主角阿喵（方案 C）：三视图 + 行走拆件 | **推荐人类画师**（如米画师上约稿"Q 版角色三视图 + 拆件"） | 美术指南第 2 节 | M7 前到位即可，想早点定形象也可以提前约 |
...
| **M2 一天循环** | 时间和天气、老宅院子和集市的场景插画、地图、鱼塘养殖、卖鱼、存档；接入 Electron | 能连续玩一周游戏日不出问题 | P1：场景插画、NPC 2~3 个 |
...
| **M6 正式发布** | 1.0（或先上抢先体验） | —— | —— |
| **M7 方案 C 行走版** | 3/4 俯视行走地图、三朝向行走动画、手柄支持 | 在村子里走来走去是一种享受 | 三视图拆件、地形纹理、3/4 物件 |
| **M8 种田更新** | 开荒、稻鱼共生、桑基鱼塘、料理 | —— | 田地相关素材、阿喵的种田动作拆件 |

**M7 的位置可以调整**：如果希望发售时就是行走版，可以把 M7 挪到 M5 之前，到 M4 时根据进度再定。
...
1. **确认**[策划大纲](game-design.md)第 18 节剩下的几个问题。
