...
| 饭店玩法 | **傍晚亲手经营**（约两分钟），不开门的日子小满代班（见 14.6） | ✅ 已定、已做 |
| 饭店名 | 喵记小馆 | ✅ 已定 |
| 菜地 | 先做小而完整的点击式菜地：6 块地、5 种作物，翻地挖蚯蚓（见第 15 节） | ✅ 已做 |
...
| 荷花湖 ✅ | 捐钱修路（¥2500） | 鲤鱼、草鱼、鳊鱼、黑鱼、翘嘴、**被放生的锦鲤（稀有）**（已做；鲢鱼、鳙鱼以后加） | 金鳞（满月夜） |
...
**荷花湖（已做）**：3 个钓位（栈桥尽头、荷花丛、芦苇边），湖底淤泥多、水浑、水面有荷叶。
湖里的鱼更大更值钱，也更难遛：鲤鱼、草鱼是耐力型，要慢慢遛；黑鱼、翘嘴是爆冲型，一直狂按必断线，
正好让"耐磨线、大物线"有用处。**被放生的锦鲤**在水里就能看见花纹，钓上来只能放进方塘养
（锦鲤只看不吃：不进活鱼缸、周叔不收），这是方塘里第三条锦鲤的来路，也是以后锦鲤育种的开头。
...
flowchart LR
    ...
    M -->|添置 · 修缮| U[升级]
    U -->|鱼护大、鱼竿好 → 一天钓得多| F
    U -->|鱼缸大、桌子多、菜牌大 → 一晚卖得多| R
    U -->|开荒、蚯蚓床 → 饵和菜不用买| G
```

**核心是一个增长回路**：钓得多 → 缸里鱼多 → 一晚卖得多 → 买升级 → 钓得更多、卖得更多。
同样一条鱼，做成菜的钱大约是直接卖给周叔的 2 倍多，所以玩家自然会去开店；
周叔的鱼摊留着是为了"今天不想开店"和"鱼护满了"的时候。

| 钱从哪来 | 大约多少 | 说明 |
|---|---|---|
| 喵记小馆（自己掌勺） | 一晚 ¥250~450 | 看缸里有几条鱼、做菜小游戏的成绩、口碑 |
| 小满代班 | 一晚 ¥40~120 | 七折，一晚只卖 3~5 道 |
| 周叔鱼摊 | 一条 ¥2~60 | 按鱼的种类和大小，大家伙贵五成 |

| 钱花到哪 | 价格 | 说明 |
|---|---|---|
| 升级（12.2） | ¥250~1800 | 主要的去处，一共约 ¥10000 |
| 种子 | ¥4~8 一颗 | 收获时有六成机会留种，基本自给 |
| 配料（阿婆杂货铺） | 葱 ¥4、面粉 ¥6、豆腐 ¥8、青菜 ¥4 | 比自己种贵，但开局菜地还没长出来时靠它 |
| 饵料 | 蚯蚓、面饵 ¥1，玉米 ¥2 | 有菜地和蚯蚓床之后基本不用买 |

开局外公的灶屋里还剩 6 根葱、4 份面粉，第一天就能开店，不用等菜地。

### 12.2 升级（已做，数值在 data/upgrades.json）

右上角"添置"（快捷键 U）。三家：

| 老韩渔具铺 | 价格 | 效果 |
|---|---|---|
| 大号鱼护 → 双层鱼护 | ¥250 → ¥700 | 鱼护 8 → 12 → 16 条 |
| 耐磨线 → 大物线 | ¥300 → ¥1000 | 鱼线强度 1 → 1.2 → 1.45（马口鱼、黄鳝不容易断线） |
| 碳素竿 → 老韩手作竿 | ¥600 → ¥1800 | 抛得更远更准，收线更快 |

| 修缮老宅 | 价格 | 效果 |
|---|---|---|
| 清塘 → 扩塘 | ¥400 → ¥1200 | 方塘 12 → 18 → 26 条 |
| 再开三块地 → 整片开出来 | ¥300 → ¥900 | 菜地 6 → 9 → 12 块（新地要自己开荒，开荒也能挖蚯蚓） |
| 蚯蚓床 | ¥500 | 每天早上多 6 条蚯蚓 |
...
### 12.3 节奏（npm run balance 的经济模拟）

模拟一个"普通玩家"过一年（56 天）：每天钓 8 条鱼（鱼竿好了多钓几条，受鱼护限制），种地，傍晚开店，
做菜成绩 0.75，按顺序攒钱添置（买不起下一项就攒着，不跳过去买便宜的）。修通小路以后去荷花湖钓。12 局平均：

| | 第 1 天 | 第 7 天 | 第 14 天 | 第 21 天 | 第 28 天 | 第 42 天 | 第 56 天 |
|---|---|---|---|---|---|---|---|
| 一天收入 | ¥300 | ¥250 | ¥370 | ¥330 | ¥620 | ¥920 | ¥890 |

- 第 1 天就买得起大号鱼护；**头两周**每 1~2 天添置一样（鱼缸、菜牌、鱼线、开荒、蚯蚓床、碳素竿、桌子、清塘）。
- **第 3 周**攒钱修路（¥2500），大约**第 26 天修通荷花湖**——这是第一年最大的一件事，
  湖里的鱼值钱，之后每天收入翻一倍多。
- 第 4~5 周把剩下的买齐，**老韩手作竿**大约第 35 天。
- 一年后口碑大约 240（口碑 3 周叔来、10 村长来、25 美食博主来）。

**之后的大件（等内容做出来再加）**：修渡船解锁大河（约 ¥6000）、锦鲤池和苗池（M5 育种）、
小馆装修的外观（纯装饰）。第 35 天以后钱只进不出，正好是这些内容接上的地方。

### 12.4 其他

- **定价**：`基础价 × 稀有度系数 × 重量系数 × 品相系数`
- **市场波动**（以后）：周叔每天的收购价有浮动，同一种鱼当天卖多了会降价，鼓励多样化。
- **技能等级**：
...
**已做**（右上角"笔记"，快捷键 J）：
- **鱼类图鉴**：每种鱼一页；没钓到的只有剪影、在哪能钓到和外公的线索，钓到后显示钓了几条、最重多少。
- **菜谱**：每道菜一页，写着用料、价钱和外公的批注，记着亲手做过几次（没做过的盘子是淡的）。
- **玩法**：一天怎么过、钓鱼、方塘、菜地、小馆、添置、快捷键，每样几句话，随时可以翻。关掉的小满便条也在这里重新打开。
- **成就**：36 个，分钓鱼、方塘、菜地、小馆、家业五类，写在 data/achievements.json。条件都是现成的数据：
  一辈子的累计记录（钓了多少鱼、挖了多少蚯蚓、亲手做了多少道菜、最好的一晚赚多少……）、外公笔记、
  鱼塘里几条鱼、口碑、升级。计数类的显示进度条；有的带一点奖励（¥20~200、蚯蚓、种子），
  给新手一个接一个的小目标。达成时右上角弹出横幅，一天的小结里也会列出来。
- 以后接 Steam 成就时按成就 id 对上后台的 API 名；育种、传说鱼、结局的成就等内容做出来再加。

### 13.1 新手引导：小满的便条（已做）

开局左上角有一张**小满的便条**，一次只写一件事，带着玩家把第一天过一遍：

| 步 | 要做的事 | 做到的条件 |
|---|---|---|
| 1 | 去屋后小溪 | 去过小溪 |
| 2 | 钓上第一条鱼 | 钓上 1 条 |
| 3 | 往鱼护里放 3 条鱼 | 放进鱼护 3 条（便条上显示 1/3） |
| 4 | 放一条鱼进自家方塘 | 放进方塘 1 条 |
| 5 | 翻翻外公笔记 | 打开过笔记 |
| 6 | 去菜地开荒，种下一颗种子 | 播过种 |
| 7 | 去小馆，把鱼放进活鱼缸 | 缸里放过鱼，并且菜单上有菜 |
| 8 | 开门营业，亲手做一道菜 | 亲手卖出 1 道菜 |
| 9 | 回家睡觉，结束第一天 | 睡过一觉 |
| 10 | 看看能添置些什么 | 打开过添置面板 |

- 每一步有小满的一句话、要做的事，和**按画面换说法的"怎么做"**：在方塘说"按 M 打开地图去小溪"，到了小溪就说怎么抛竿、提竿、遛鱼。
- 顺序是建议，不是锁：提前做过的事也算，走到那一步会直接跳过去。做到一步闪一下"做到了！"，最后小满说声再见，便条自己收起来。
- 便条可以收起（只留一行），也可以关掉；关掉以后在外公笔记的"玩法"页能重新打开。
- 步骤和说法都写在 data/tutorial.json，条件和成就共用同一套（记录、鱼、升级……），另外加了两种：
  **做过的一次性小事**（去过哪、打开过什么，存档里的 flags）和**菜单上挂了几道菜**。
- 老存档读进来从第一步开始，做过的步骤自动跳过；钓过鱼就算去过小溪，买过升级就算打开过添置面板。
...
定为 **A，并保留 B 作为"让小满代班"的选项**：想做菜的日子自己掌勺，想专心钓鱼的日子交给小满。

**已做的版本**（占位画面，数值在 data/restaurant.json、data/recipes.json）：
- 17:00 开门，19:40 以后不进新客人，20:00 打烊；白天可以"在店里忙到傍晚"直接跳到 17:00。
- 4 张桌子、8 个座位；口碑为 0 时每 8~15 分钟来一位客人，口碑越高来得越勤。
- 客人只点菜单上、此刻做得出来的菜（材料点菜时就预留）；兔子吃素只点素菜；等 45 分钟还没上菜就先走，不扣钱。
- 做菜小游戏三步：切配、掂锅、装盘，每步在好区按空格。成绩 0~1 决定价钱（0.75~1.25 倍）、小费和口碑。
- 大鱼做的菜更贵；做鱼菜攒下的鱼杂两份变一份堆肥。
- 小满代班：一晚卖 3~5 道（口碑高了多卖），价钱七折；早早睡觉也照样代班。
- 一晚上的收入参考（npm run balance）：缸里 8 条鱼、做得不错，大约接待 11 位、¥300；同样的鱼直接卖给周叔约 ¥130。

### 14.7 打烊（已做）

- 营业中随时可以"提前打烊"；菜卖完、店里空了、鱼护里也没鱼，等一会儿自动打烊。
- 菜卖完时提示"鱼护里还有 N 条鱼，放进活鱼缸就能接着卖"。
- 打烊弹出**今晚的账本**：接待几位、卖了哪些菜、收入和小费、做得特别好的几道、口碑涨了多少、
  口碑到多少会有新客人来、一位客人的话，以及接下来可以做什么（回家睡觉、去小溪夜钓、在店里待会儿）。
- 这是"一天做完"的收尾：辛苦钓的鱼变成了什么、赚了多少、离下一个目标还有多远，都在这一张纸上。
...
| 作物 | 季节 | 天数 | 用处 |
|---|---|---|---|
| 玉米 | 春、夏 | 6 | 鲜粒做饵；晒干磨碎做窝料；小馆的玉米烙 |
| 小麦 | 春、秋 | 7 | 磨面粉 → **面饵**、面条 |
| 黄豆 | 春、夏 | 5 | 做豆腐（小馆）；豆饼做窝料 |
| 葱 | 四季 | 3 | 小馆配料 |
| 青菜 | 春、秋、冬 | 3 | 小馆配料；喂草鱼 |

游戏从春天开始，所以前几种都能在春天种；天数比真实短很多，好让玩家一周之内就能收上几轮。
收获时有六成机会留下一颗种子，没钱买种子也不会断。（数值在 data/crops.json）
