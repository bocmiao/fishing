...
> - [美术与音频指南](art-audio-guide.md)：可以直接交给图像 AI / 音乐 AI 的风格说明、资源清单和提示词
...
| 视角 | **全俯视、场景式，没有可行走的主角**（见第 4 节） | 🟡 建议，待确认 |
| 技术栈 | **TypeScript + PixiJS + Electron**（替代之前建议的 Godot，理由见技术方案） | 🟡 建议，待确认 |

---

## 1. 游戏概述

**一句话**：回到乡下外公的老宅，在溪边湖畔钓鱼，把荒废的方塘养成一池锦鲤，最终钓到外公一生都没钓到的那条鱼。
...
比较准确的叫法是 **"国风治愈 · 扁平水彩插画"**（英文提示词：flat illustration, watercolor texture, muted teal palette, Chinese minimalist）。

这个风格对我们特别有利：**鱼、水波、光斑、天气都可以用代码程序化绘制**，只有池底、场景、人物等"画"需要图像 AI 出图。具体规则和提示词见[美术与音频指南](art-audio-guide.md)。
...
- **舞台**：南方山间的一个小村（虚构），有溪、有荷塘、有大河渡口，四季分明。时代是现代，带一点怀旧感（老收音机、竹竿和碳素竿并存）。
- **开场**：外公去世后，玩家回到老宅，发现屋后的方塘早已干涸长草，旁边还有一块荒地。屋里留着一本**外公的钓鱼笔记**。
- **主线**：外公笔记既是图鉴，也是故事线。笔记里记着每一种鱼的线索，最后几页写着几条"传说中的鱼"，其中一条外公钓了一辈子都没钓到。
- **结局**：钓到"外公的那条鱼"后，玩家可以选择放生它。游戏在结局后继续，没有终点。
- **种田伏笔**：荒地从第一天起就在家园图里，被杂草和石头覆盖。后期版本开荒即可种田（见第 13 节）。
...
所有场景都是**正上方俯视**，像从高处看一幅水面的画，**不设可行走的主角**：

- **家园图**：一张俯瞰的"庭院图"，有老宅屋顶、方塘、荒地、竹林和小路。点击区域进入对应场景。
- **鱼塘**：就是参考图那样的画面，锦鲤在池中群游。
- **钓点**：同样是俯视水面，玩家从岸边往水里抛竿，看得到鱼影游过来咬钩。
- **地图**：一张手绘风的村子地图（像夹在外公笔记里的那种），点击前往各地点，路上会消耗游戏时间。

**这样做的好处**：

1. 美术量大幅减少：不需要角色行走动画、不需要大量地图素材。
2. 钓鱼和养鱼**共用同一套"水面 + 鱼"的渲染技术**，一套技术做好，两个核心玩法都好看。
3. 风格统一，和参考图的气质完全一致。
...
| 熬夜 | 到 2:00 自动回家睡觉，**没有惩罚** |
| 睡觉 | 触发每日结算：鱼塘生长和繁殖、水质变化、钓点鱼情恢复、订单刷新、写鱼塘日志 |
| 季节 | 每季 14 天，一年 56 天（可调） |
| 天气 | 晴 / 多云 / 雨 / 雷雨 / 雾 / 雪（冬），老宅里的**旧收音机**能听到明天的天气预报 |

时间本身就是资源：清晨、黄昏、夜里出的鱼不同，下雨天有的鱼特别活跃。玩家每天要决定"今天去哪钓、钓多久、什么时候回家喂鱼"。

### 4.3 一天的样子（示例）

> 6:00 起床，听收音机：明天有雷雨，大河的"墨鲤"可能会出现 →
> 6:30 喂锦鲤、捞落叶 → 7:00 去小溪钓清晨的马口鱼 →
> 11:00 到村里把鱼卖给周叔、在公告栏交一个订单 →
> 午后在荷花湖钓鱼，钓到一条别人放生的红白锦鲤！→
> 18:00 回家，把锦鲤放进池子，给它起名"小满" →
...
| 步骤 | 玩法 | 技巧空间 |
|---|---|---|
| **1. 观察** | 水里只看得到鱼的**影子**，按大小分为小 / 中 / 大 / 巨四档；水花、气泡提示鱼群；浅滩、深水、水草、石缝出的鱼不同 | 认影子、找好位置 |
| **2. 抛竿** | 鼠标指向目标点，按住蓄力，松开抛出；落点有随机偏差（竿越好越准） | 抛在鱼的前方而不是头顶，**砸到鱼头会把它吓跑** |
| **3. 诱鱼** | 鱼察觉饵料后游过来，绕饵、试探（浮漂轻颤），最后咬钩（浮漂猛沉 + 提示音） | 饵料对不对胃口，决定鱼来不来 |
| **4. 提竿** | 咬钩后的短暂窗口内点击；太早提竿会惊走鱼 | 狡猾的鱼窗口更短 |
| **5. 遛鱼** | 张力小游戏（见 6.2），俯视画面里能看到鱼影拖着鱼线乱窜 | 核心手感 |
| **6. 上鱼** | 弹出鱼的卡片：名称、重量、稀有度，提示"新纪录""笔记新页"；选择**放进鱼护**或**放生** | |
...
| 渔具 | 影响 | 成长线（示例） |
|---|---|---|
| 鱼竿 | 抛投距离、精度、张力上限 | 老竹竿 → 碳素竿 → 老韩手作竿 → 外公的竿（剧情） |
| 鱼线 | 断线阈值 | 普通线 → 耐磨线 → 大物线 |
| 浮漂 | 咬钩提示清晰度 | 普通漂 → **夜光漂（夜钓必备）** → 灵敏漂（小鱼咬钩更明显） |
| 鱼钩 | 脱钩概率 | |
| 饵料 | 决定吸引哪类鱼 | 买 / 挖（后期种田自产） |
| 窝料 | 打窝后一段时间内某类鱼出现率提高 | 后期可用自种的玉米、豆子做 |

### 6.6 钓点与鱼种（1.0 目标约 40 种）
...
- 扩建后分为三类：
  - **锦鲤池**：观赏、育种、参加品评会，就是参考图那样的画面，是游戏的"门面"。
  - **经济塘**：养草鱼、鲫鱼等，稳定卖钱。
  - **苗池**：繁殖孵化、挑选鱼苗。
...
**品种由外观规则自动判定**：白底 + 红斑 = 红白；再加少量墨斑 = 大正三色；黑底 + 红 + 白 = 昭和三色；全身金属黄 = 黄金……约 15 个品种，而每一条鱼的具体花纹都不一样。
...
- **观鱼模式**：一键隐藏所有界面，只剩鱼塘、环境声和音乐，可以当作放松或工作时的背景。（后期评估是否加"桌面小窗置顶"模式，Steam 上这类桌面挂机游戏很受欢迎。）
- **装饰**：睡莲、荷花、菖蒲、石灯笼、小石桥、汀步、假山、竹筒流水（惊鹿）、水车、乌龟……大部分纯观赏，少量有功能（水草改善水质、荷叶在夏天遮阳降温）。
- **鱼塘日志**：每天结算时自动生成几条趣事，比如"小满今天抢到了最多的鱼食""新孵出 18 条小鱼""一只白鹭在塘边站了一下午"。开发成本低，但很有陪伴感。
...
NPC 控制在 6 个左右，只有立绘和对话，没有行走动画：

| NPC | 身份 | 功能 |
|---|---|---|
| 周叔 | 鱼贩 | 收鱼，每天报价不同 |
| 阿婆 | 杂货铺 | 饵料、饲料、药品（后期卖种子） |
| 老韩 | 渔具匠，外公的老钓友 | 渔具升级；讲外公的往事；给山涧地图 |
| 小满 | 邻居家的孩子 | 新手引导；出图鉴挑战（"你能钓到比我还大的鲫鱼吗？"） |
| 林先生 | 城里来的锦鲤藏家 | 卖第一条锦鲤；高价收购精品锦鲤；品评会引路人 |
| 村长 | 村长 | 公告栏订单；出钱修路、修渡船，解锁钓点 |
...
```mermaid
flowchart TD
    S[第 1 天：老竹竿 + 屋后小溪] --> P[攒钱修复方塘，开始养鱼]
    P --> L[村长委托：修小路，解锁荷花湖]
...
- **Steam 成就（约 30 个）**：第一条鱼、笔记完成 25% / 50% / 100%、第一次繁殖、培育出丹顶、品评会冠军、钓齐 5 条传说鱼、结局……

---

## 13. 后期扩展：种田（围绕"水"与鱼塘共生）

和星露谷式的种田拉开差异，**用中国传统的生态农法作为核心玩法**，天然契合国风主题：

| 联动 | 玩法 |
|---|---|
| **稻鱼共生** | 在稻田里放养鱼苗：鱼吃虫除草、鱼粪肥田，水稻增产，秋收时还能收获一批"稻花鱼" |
...
技术上，种田复用现有系统：作物和鱼共用"生长系统"，种子、作物、菜肴都是物品系统里的物品，时间和季节已经现成。
...
| 风险 | 应对 |
|---|---|
| 程序化绘制的鱼能否达到参考图的质感 | 第一步（M0）先做技术验证，只做一个鱼塘画面 |
| 图像 AI 出图风格不统一 | 先定一张主视觉，其余全部以它为风格参考；游戏内再加统一调色 |
| 钓鱼手感不好玩 | 尽早做出钓鱼原型，找人试玩，反复打磨 |
| 内容太多做不完 | 严格按里程碑推进，1.0 之前不做种田 |

## 16. 待确认

1. 视角方案（全俯视、无行走主角）是否认可？
2. 技术栈改为 TypeScript + PixiJS + Electron 是否认可？
3. 游戏名《半亩方塘》是否喜欢？
4. 是否需要支持手柄 / Steam Deck？（会影响操作设计，越早定越好）
