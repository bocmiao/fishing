...
网址参数：`?scene=pond|fishing|lake|farm|restaurant&seed=7&warmup=10&debug=1&fresh=1`（`shot=1` 为截图模式，由截图脚本使用，不读也不写存档）；网页版拿不到网址参数，用 `#fishing` 这样的锚点直接进某个画面，`#new` 从头开始。
...
src/scenes/   各个画面（鱼塘、钓鱼（小溪和荷花湖共用）、菜地、小馆），组合 sim 和 render
...
3. **内容写进配置表**（鱼、渔具、钓点、作物、菜谱、客人、地点、升级、成就、新手引导……），不要写死在代码里。
